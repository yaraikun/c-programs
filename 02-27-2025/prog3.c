...
void PrintStudentData(struct studentTag S[])
{
    ...
    for (i = 0; i < 12; i++) {
        printf("$$$ %d %-15s %-15s %.2f\n", 
             S[i].id_number,  S[i].first_name,  S[i].last_name, S[i].grade); 
    }
...
int main()
{
    struct studentTag STUDENTS[MAX_STUDENTS];  // Chapter 1 combined with Chapter 3
    // each array element is of type struct studentTag
